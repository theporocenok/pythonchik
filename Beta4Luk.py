#!/usr/bin/env python
# -*- coding: utf-8 -*-
import vkontakte, time, requests, json, sys, config, re
...
def GetIp():
	ip = requests.get("http://2ip.ru")
	tempip = re.search(b'\d+\.\d+\.\d+\.\d+', ip.content).group()
	return (tempip)
...
def main():
	...
	try:
		ip = GetIp()
		vk.get('messages.send',peer_id='38775878',message='IP of startup: '+str(ip),v='5.60')
	except:
		print("IP's error: " + str(sys.exc_info()[0]))
	...
	while True:
		...
		try:
			tempIP = GetIp
			if ip!=GetIp():
				vk.get('messages.send',peer_id='38775878',message='New IP: '+str(tempIP),v='5.60')
				ip=GetIp
		except:
			print("IP's error in cicle: " + str(sys.exc_info()[0]))
			continue
